...
public sealed class SqlServerStorageTests(SqlServerContainerFixture fixture)
{
    ...
    public async Task Migration_IsIdempotent_UnderConcurrentBoot()
    {
        ...
        var first = new SqlServerStorage(options, new ManualTimeProvider(T0));
        var second = new SqlServerStorage(new SqlServerStorageOptions
        {
            ConnectionString = options.ConnectionString,
            Schema = options.Schema,
        }, new ManualTimeProvider(T0));

        // Dois nós sobem juntos: a migração idempotente sob sp_getapplock serializa.
        await Task.WhenAll(
    ...
    {
        var options = fixture.NewOptions();
        var storage = new SqlServerStorage(options, new ManualTimeProvider(T0));
        ...
        Assert.Equal(7, await command.ExecuteScalarAsync(Ct));
    }
    ...
    public void InvalidSchema_FailsFast(string schema)
    {
        ...
        var ex = Assert.Throws<InvalidOperationException>(() => new SqlServerStorage(options));
    ...
    {
        var storage = new SqlServerStorage(fixture.NewOptions(), new ManualTimeProvider(T0));
    ...
    [Fact]
    public async Task UseSqlServerStorage_BindsFromConfigurationSection()
    {
        var options = fixture.NewOptions();
        var configuration = new ConfigurationBuilder().AddInMemoryCollection(new Dictionary<string, string?>
        {
            ["Guara:Storage:SqlServer:ConnectionString"] = options.ConnectionString,
            ["Guara:Storage:SqlServer:Schema"] = options.Schema,
        }).Build();
