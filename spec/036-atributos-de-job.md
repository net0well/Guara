...
> **Implementação (2026-07-19):** os 5 atributos entregues em `Guara.Abstractions` e lidos **em compilação** pelo generator (spec 029), que os materializa em `JobExecutionMetadata` no registro — zero `GetCustomAttributes` em runtime (AC-7); método vence classe (AC-8, o parser aplica classe→método). Realização no runtime: **`[GuaraFila]`** entra no descriptor tipado na criação (AC-1); **`[GuaraRetentativas]`** sobrepõe `RetryOptions.MaxAttempts` na decisão de falha do executor (AC-2 — a decisão saiu do RetryMiddleware junto com a retentativa persistente, spec 008); **`[GuaraTempoLimite]`** vira CTS linkado no executor — token honrado = falha "tempo limite" que **segue a política de retentativa** (AC-5 vale ao esgotar; uniformidade decidida aqui), token ignorado + completou = `Succeeded` + aviso (semantics 2026-07-17); **`[GuaraDesabilitarConcorrencia]`** virou **gate no executor** (não middleware — evita ordenação frágil com diagnostics e exceção de controle): `TryAcquireAsync` no `ILockProvider` com chave `guara:mutex:{chave}` (TTL 10min; crash libera por expiração), ocupado → `IJobStorage.RescheduleAsync` devolve à fila **sem consumir tentativa** (novo na spec 004), `EsperaSegundos` espera cooperativa antes de devolver (DD-1 ✓); chave com placeholders formatada por delegate **gerado** com os argumentos desserializados (AC-4); **`[GuaraPularSeAnteriorEmExecucao]`** materializado nos metadados — a ponte para `SkipIfPreviousRunning` do recorrente liga quando o recorrente tipado chegar (spec 038/builder + factory gerada). AC-3 (exclusão entre nós) coberto por teste com lock ocupado + provider PostgreSQL (locks distribuídos reais).
