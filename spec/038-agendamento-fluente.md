...
- **`IGuaraClient`** ganha (extend-only): `AdicionarOuAtualizarRecorrenteAsync(Action<IRecurringJobBuilder>, ct)`, `ExcluirRecorrenteAsync(id, ct)` (remove a definição sem afetar ocorrências já enfileiradas/rodando) e `AdicionarOuAtualizarCalendarioAsync(nome, Action<ICalendarBuilder>, ct)` / `ExcluirCalendarioAsync(nome, ct)`. Semântica de recorrentes (sobreposição, misfire, pausa) canônica em [semantics.md](../docs/semantics.md).
...
> **Implementação (2026-07-19) — recorrente tipado:** `Executa(...)` aceita naturalmente a **factory gerada** (`job.Executa(RelatorioJobsGuara.GerarAsync(42))` — spec 029), e o builder passou a honrar a marca `guara-pular-se-anterior` que a factory emite quando o job declara `[GuaraPularSeAnteriorEmExecucao]`: o `SkipIfPreviousRunning` liga sozinho, sem exigir a chamada fluente (chaves conhecidas em `JobMetadataKeys`, Abstractions).

> **Implementação (2026-07-18):** entregue conforme o contrato. Builder `RecurringJobBuilder` valida tudo no `Build` (AC-9: id/job obrigatórios, exatamente uma agenda, `EntreHorarios` só com `ACada`, cron re-parseado para mensagem detalhada, `TerminaEm` > `IniciaEm`, fuso resolvido — IANA e Windows nativos, AC-7). `GuaraDatas` com sobrecargas de fuso em UTC por default (DD-3, AC-8). Calendários com as quatro regras do 1.0 (DD-2) e recálculo automático do `NextRunAt` de quem usa o calendário alterado (AC-6); exclusão de calendário em uso bloqueada listando os recorrentes. Janela diária (DD-1): implementada, cruza a meia-noite quando início > fim; grade do intervalo ancorada em `IniciaEm` (AC-3); upsert preserva histórico e recalcula o próximo disparo a partir de agora (AC-2). **Fora deste incremento:** `Executa(() => ...)` tipado — depende do source generator (specs 019/029), hoje a forma é `Executa(JobDescriptor)`; AC-10 depende dos atributos da spec 036 (ainda não implementados). Cobertura: `RecurrenceCalculatorTests`, `RecurringClientTests`, `GuaraDatasTests`, conformance de `Recurring`/`Calendars` e dois e2e no host (promoção repetida e pulo por sobreposição).
