...
> **Implementação (2026-07-19):** entregue no pacote `Guara.Dashboard`. **Regras fluentes** (`UseGuaraAuthentication` como método de `DashboardOptions`, conforme o encadeamento do contrato): `PermitirApenasLogados`, `ExigirPapel(params)`, `ExigirClaim(tipo[, valor])`, `PermitirApenasIpsInternos` (RFC 1918 + loopback + fc00::/7 + link-local, com IPv4-mapeado normalizado), `PermitirIps(CIDR...)` (validação **na chamada**), `ComLoginFixo` (implica logados; **proibido dentro de `QualquerUma`**), `ComRegra<T>`/`ComRegra(instância)` e `QualquerUma(...)` (OU); combinação default em E (AC-5/AC-6). **`IDashboardAccessRule`/`DashboardContext`** com `HttpContext` completo (AC-7). Avaliação num **endpoint filter** no grupo protegido (API + SSE + UI — a conexão SSE é autorizada na abertura); exceção em regra = negado + log (AC-10). **Login fixo (AC-8):** senha vira hash **PBKDF2-SHA256 (100k)** na própria chamada (nunca retida/logada), comparação em tempo constante (usuário e senha), cookie **HMAC-SHA256 assinado** HttpOnly/SameSite=Strict/Secure-em-HTTPS com validade (`SessionTtl`, default 8h; segredo configurável via `Guara:Dashboard:CookieSecret` — sem ele, chave por boot), **rate limit por IP** (5 falhas/5min → lockout 5min, DD-3 fallback) com tentativas logadas sem a senha, `retorno` sanitizado contra open redirect, e **avisos de startup** (login fixo → recomenda env/secret; RequireAuthorization=false → aviso forte). **Página de login (AC-9):** embutida no pacote (a SPA da 024 poderá substituí-la), logo do Guará, claro/escuro via `prefers-color-scheme`, pt-BR/en via `Accept-Language`, labels/roles ARIA básicos. Negado: navegador anônimo → redirect ao login (com login fixo) ; API → 401/403 `ProblemDetails` (AC-1/AC-2). *Antiforgery do form de login:* mitigado por SameSite=Strict; token antiforgery entra junto com a SPA (anotação honesta).
