...
> - **Conformance kit** implementado como classe abstrata `StorageConformanceTests` (aquisição atômica concorrente, lease/visibility, retentativa persistente, idempotência, retenção/purga, servidores, recorrentes/calendários, continuações, locks TTL e paginação). **Extraído (2026-07-19, na chegada do 2º provider)** para a classlib compartilhada `tests/Guara.Storage.Conformance` (xunit.v3.extensibility.core + assert; storages descartáveis são liberados por teste) — herdada por Memory e PostgreSql; publicação como pacote NuGet para providers de terceiros é decisão da spec 033.
...
> - **`IJobStorage.ScheduleRetryAsync(id, error, retryAt)`** — retentativa persistente em uma operação atômica: `Retrying` + motivo + `Attempt` incrementado + `ScheduledFor = retryAt` + posse liberada; job inexistente é ignorado. A **elegibilidade** de `AcquireNextDueAsync` passou a incluir `Retrying` vencido (`ScheduledFor <= now`), preservando o `Attempt` na aquisição.
> - **`IStorage.Continuations`** (**`IContinuationStorage`**) com a estrutura **`Continuations`** (`ContinuationRecord`: chave = `ChildId`, `ParentId`, `Trigger`, `Status` Pending/Enqueued/Discarded, `Reason`, `Depth`, `CreatedAt`/`ResolvedAt`). `AddAsync` idempotente pelo filho; **`TryResolveAsync` é o ponto de idempotência do disparo** — só transiciona uma vez a partir de `Pending`, então entre nós concorrentes apenas um vence; `ListPendingAsync` alimenta a varredura de recuperação e `ListByParentAsync` o fan-out sem N+1. Conformance kit cobre as duas famílias.
