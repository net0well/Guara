...
> **Implementação (2026-07-19) — E1 da fase do dashboard:** pacote entregue com o contrato da tabela + `GET /recurring` (recurso do domínio): stats (novo `IJobStorage.CountByStateAsync`, spec 004), queues, jobs paginado com filtros e **teto aplicado** (AC-2), detalhe (404 `ProblemDetails`), servers, retry (só `Failed` → `RescheduleAsync(now)` preservando `Attempt` — nova falha volta a Failed, histórico honesto; 409 fora disso — AC-6), trigger (`Scheduled`/`Retrying` → agora), delete (via `ExcluirAsync` — descarta continuações pendentes; 409 quando `Processing`) e o **stream SSE** (AC-8): `DashboardEventStream` assina o event bus por forwarders tipados, canal limitado por cliente (`DropOldest` — cliente lento nunca trava o publicador), `id:` sequencial para `Last-Event-ID`, keep-alive `: ping` a cada 15s e `X-Accel-Buffering: no`. DTOs planos camelCase com contexto STJ gerado; `TypedResults` + metadados OpenAPI em tudo (AC-1). **Pendências para a E2 (composição/auth):** autorização granular `guara:*` (specs 020/021/037) — o `MapGuaraDashboardApi()` devolve o grupo e a **composição aplica `RequireAuthorization` por padrão** (deny-by-default realizado lá; montar o grupo cru é explicitamente documentado como exposto); payload de argumentos fora do detalhe até existir `guara:view-payload` (AC-4 satisfeito por omissão total); documento OpenAPI (`AddOpenApi/MapOpenApi`) é wiring do host — os metadados já alimentam (AC-5 completa na composição); rate limiting (DD-3) idem. **AOT:** pacote marcado `IsAotCompatible=false` com justificativa (o binding de Minimal API em biblioteca usa o caminho reflection; o RDG só roda no app) — o núcleo permanece AOT e quem publica AOT roda headless. Multi-nó: o stream reflete o bus local até o push do storage (DD-1).
