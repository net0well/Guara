...
/// <summary>
/// Opções do storage PostgreSQL. Configuráveis pela seção
/// <c>Guara:Storage:PostgreSql</c> (a connection string é um segredo: prefira
...
public sealed partial class PostgreSqlStorageOptions
{
    ...
    /// <summary>
    /// Aplica o esquema idempotente no primeiro uso (coordenado por advisory lock
    ...
    internal void Validate()
    {
        if (string.IsNullOrWhiteSpace(ConnectionString))
        {
            throw new InvalidOperationException(
                "PostgreSqlStorageOptions.ConnectionString é obrigatória. Informe-a em " +
                "UsePostgreSqlStorage(...) ou na seção Guara:Storage:PostgreSql:ConnectionString.");
        }
        ...
        {
            throw new InvalidOperationException(
                $"PostgreSqlStorageOptions.Schema inválido: '{Schema}'. Use apenas letras minúsculas, " +
